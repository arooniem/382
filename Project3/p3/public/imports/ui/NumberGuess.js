...
export default class NumberGuess extends Component {
	...
	getRESTData(min, max) {
  	...
  	handlePlayButton(event){
  		//set state to display loading on play button
  		...
  		//cheat
  		console.log(rando);*/
  		//rest call for random integer
  		this.getRESTData(this.state.lowRange, this.state.highRange);
				...
				return( 
					<Segment inverted color='grey'>
						<Segment inverted color='grey' raised>
							...
							<span>
								<Input 
									onChange={this.handleGuessInput.bind(this)} 
									ref={this.handleRef.bind(this)} 
									//Semantic-ui button tied with Input
									action={{
										content:'Guess', 
										onClick:this.handleGuessButton.bind(this), 
										primary:true,
									}} 
								/>
